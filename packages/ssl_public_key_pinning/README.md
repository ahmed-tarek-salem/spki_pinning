...
HTTP client, fail-closed, with backup-pin support. Pure Dart: no platform
channels, no native code, works on Android, iOS, desktop, and server-side
Dart.
...
| **`ssl_public_key_pinning`** | ✓ | ✓ Dio, http, or raw `HttpClient` |
...
### With Dio - [`ssl_public_key_pinning_dio`](https://pub.dev/packages/ssl_public_key_pinning_dio)
...
### With package:http - [`ssl_public_key_pinning_http`](https://pub.dev/packages/ssl_public_key_pinning_http)
...
```dart
final pinning = PublicKeyPinning(pins: {
  'api.example.com': ['sha256/AAAA...=', 'sha256/BBBB...='],
});
...
```bash
dart run ssl_public_key_pinning:pin api.example.com
```
...
`dart run ssl_public_key_pinning:pin` tool measures through the same TLS stack Dart
...
- Unpinned hosts are only CA-validated when reached through the routing
  bindings (`ssl_public_key_pinning_dio` / `ssl_public_key_pinning_http`); the raw pinned
...
- Dio: `DioException` with type `badCertificate` and
  `error is PublicKeyPinningException`.
- http: `PublicKeyPinningException`.
- Raw `HttpClient`: `HandshakeException` (the `dart:io` callback API only
  carries a boolean); details arrive via the observer.

Malformed pins throw `ArgumentError` at `PublicKeyPinning` construction - bad
